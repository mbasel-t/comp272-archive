 ...
 */
 ...
 public class BinaryTree {
     ...
     private void replaceValueHelper(Node node, int oldVal, int newVal) {
         ...
         Node target = findNode(node, oldVal);
         if (target != null)
            target.data = newVal;
     }
     ...
     private int nodesGTHelper(Node node, int val) {
        ...
        return 0 + (node != null ? (node.data > val ? 1 : 0) + nodesGTHelper(node.left, val) + nodesGTHelper(node.right, val) : 0);
     }
     ...
     public double average() {
         ...
         return (double) sumAndCount[0] / Math.max(sumAndCount[1], 1);
     }
     ...
     private int[] averageHelper(Node n) {
         ...
         if(n != null) {
            ...
            sumAndCount[0] += n.data + leftSumAndCount[0] + rightSumAndCount[0];
            sumAndCount[1] += 1 + leftSumAndCount[1] + rightSumAndCount[1];
         }
         return sumAndCount;
    }
}