 ...
 class LUC_AVLTree {
     ...
     private Node deleteElement(int value, Node node) {
        ...
        if (node != null) {
            ...
            // PART 1: REMOVING NODE
            if (node.value == value) {

                // NODE IS TO BE REMOVED; CHECK FOUR CASES
                if (node.rightChild == null) {
                    if (node.leftChild == null) {
                        // CASE 1: LEAF NODE
                        return null;
                    } else {
                        // CASE 2: INTERIOR w/ ONLY LEFT SUBTREE
                        node = node.leftChild;
                    }
                } else {
                    if (node.leftChild == null) {
                        // CASE 3: INTERIOR w/ ONLY RIGHT SUBTREE
                        node = node.rightChild;
                    } else {
                        // CASE 4: INTERIOR w/ TWO SUBTREES
                        Node successor = minValueNode(node.rightChild);
                        node.value = successor.value;
                        node.rightChild = deleteElement(node.value, node.rightChild);
                    }
                }
            } else if (node.value > value) { // NODE IS NOT TO BE REMOVED; CONTINUE SEARCHING
        ...
        // PART 2: BALANCING
        if(getBalanceFactor(node) > 1) { // left imbalance
            if(getBalanceFactor(node.leftChild) > 0) {
                node = LLRotation(node);
            } else {
                node = LRRotation(node);
            }
        } else if (getBalanceFactor(node) < -1) { // right imbalance
